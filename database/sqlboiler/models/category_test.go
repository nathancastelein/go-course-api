...
func testCategoryToManyPets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.Category, a.ID)
	queries.Assign(&c.Category, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.Category, b.Category) {
			bFound = true
		}
		if queries.Equal(v.Category, c.Category) {
			cFound = true
		}
	}
	...
	if err = a.L.LoadPets(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Pets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Pets = nil
	if err = a.L.LoadPets(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Pets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCategoryToManyAddOpPets(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.Category) {
			t.Error("foreign key was wrong value", a.ID, first.Category)
		}
		if !queries.Equal(a.ID, second.Category) {
			t.Error("foreign key was wrong value", a.ID, second.Category)
		}

		if first.R.PetCategory != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.PetCategory != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.Pets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Pets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Pets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testCategoryToManySetOpPets(t *testing.T) {
	var err error
	...
	err = a.SetPets(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.Category) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.Category) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.Category) {
		t.Error("foreign key was wrong value", a.ID, d.Category)
	}
	if !queries.Equal(a.ID, e.Category) {
		t.Error("foreign key was wrong value", a.ID, e.Category)
	}

	if b.R.PetCategory != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.PetCategory != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.PetCategory != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
	if e.R.PetCategory != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
...
func testCategoryToManyRemoveOpPets(t *testing.T) {
	var err error
	...
	err = a.AddPets(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemovePets(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	categoryDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`}
	_               = bytes.MinRead
)
