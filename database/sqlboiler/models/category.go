...
type categoryR struct {
	Pets PetSlice `boil:"Pets" json:"Pets" toml:"Pets" yaml:"Pets"`
}
...
func (r *categoryR) GetPets() PetSlice {
	if r == nil {
		return nil
	}
	return r.Pets
...
func (o *Category) Pets(mods ...qm.QueryMod) petQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"pet\".\"category\"=?", o.ID),
	)

	return Pets(queryMods...)
}

// LoadPets allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (categoryL) LoadPets(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCategory interface{}, mods queries.Applicator) error {
	var slice []*Category
	var object *Category
	...
	query := NewQuery(
		qm.From(`pet`),
		qm.WhereIn(`pet.category in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.Category) {
				local.R.Pets = append(local.R.Pets, foreign)
				if foreign.R == nil {
					foreign.R = &petR{}
				}
				foreign.R.PetCategory = local
				break
			}
		}
	}
...
// Sets related.R.PetCategory appropriately.
func (o *Category) AddPets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Pet) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.Category, o.ID)
...
// Sets o.R.PetCategory's Pets accordingly.
// Replaces o.R.Pets with related.
// Sets related.R.PetCategory's Pets accordingly.
func (o *Category) SetPets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Pet) error {
	query := "update \"pet\" set \"category\" = null where \"category\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Pets {
			queries.SetScanner(&rel.Category, nil)
			if rel.R == nil {
				continue
			}

			rel.R.PetCategory = nil
		}
		o.R.Pets = nil
	}

	return o.AddPets(ctx, exec, insert, related...)
}
...
// Sets related.R.PetCategory.
func (o *Category) RemovePets(ctx context.Context, exec boil.ContextExecutor, related ...*Pet) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("category")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(categoryPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert category, could not build conflict column list")
			}

			conflict = make([]string, len(categoryPrimaryKeyColumns))
			copy(conflict, categoryPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"category\"", updateOnConflict, ret, update, conflict, insert, opts...)
